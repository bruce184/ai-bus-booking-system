...
export async function callTool(name, args = {}) {
  switch (name) {
    ...
    case "get_revenue_summary":
      ...
      return queryGraphqlText({
        query:
          "query AdminRevenueSummary($input: AdminRevenueSummaryInput!) { adminRevenueSummary(input: $input) { from to totalRevenue paidBookings ticketsSold successfulBookingRate } }",
        variables: { input: { from: args.from, to: args.to } },
        admin: true
      });
    case "get_popular_routes":
      return queryGraphqlText({
        query:
          "query PopularRoutes($limit: Int) { popularRoutes(limit: $limit) { origin destination searchCount } }",
        variables: { limit: args.limit ?? 5 }
      });
    default:
